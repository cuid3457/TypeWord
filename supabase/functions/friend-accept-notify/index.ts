...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // pointing at the requester. accept_friend_request inserts this row,
    // so its presence proves the acceptance actually happened.
    const { data: friendship } = await admin
      ...
      .maybeSingle();
    if (!friendship) return jsonResponse(403, { code: "no_friendship" }, cors);
