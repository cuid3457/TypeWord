 ...
 * model in three narrow cases where gpt-4.1-mini reliably misbehaves:
 *   1. Short Latin-script words (≤5 chars, cross-language homograph leaks).
 *   2. CJK reduplicated 2-char words (compositional-meaning leaks).
 *   3. Short CJK words (1–3 chars) — mini frequently false-refuses these
 *      with `note='non_word'` despite them being valid vocabulary, because
 *      the strong anti-fabrication rule + minimal input context tips the
 *      model toward refusal. 4+ char CJK inputs (e.g. four-char idioms,
 *      longer compounds) carry enough signal that mini stays accurate.
...
export function selectModelForLookup(req: WordLookupRequest): string {
  ...
  if (CJK_LANGS.has(req.sourceLang) && [...w].length <= 3) return HIGH_QUALITY_MODEL;
 ...
 * PROMPT_VERSION bumps invalidate all cached results when the prompt changes
 * shape enough that prior outputs are wrong (e.g. new sense-discovery rule).
 */
...
export function buildCacheKey(req: WordLookupRequest, model?: string): string {
  ...
  return `${normWord}|${req.sourceLang}-${req.targetLang}|${mode}|${PROMPT_VERSION}|${m}${rh}`;
}
