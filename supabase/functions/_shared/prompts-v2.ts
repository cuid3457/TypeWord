...
// Positional list: index N in any language's list maps to index N in
// every other language's list. Append-only — never reorder positions
// 0–9 (existing cached entries rely on that order via POS_BY_LANG
// reverse lookup in stitch.ts).
//
// Position 10 = numeral, Position 11 = symbol.
// Before 2026-05-19 numbers were collapsed onto "noun"/"expression" via
// stitch.ts POS_ALIASES — now they have their own canonical slot.
...
STANDALONE-EXAMPLE TEST (CRITICAL — strict gate BEFORE inclusion-favored rule):
- A meaning belongs in the meanings array ONLY IF a learner-friendly single-clause example sentence can demonstrate that exact sense using the bare headword AS the headword itself (not as a constituent inside a larger compound word).
- If the sense exists ONLY inside fixed phrases / phrasal verbs / idioms / collocations / COMPOUND WORDS (e.g. "account = consider" only works as "take into account"; "make = cause" only in "make + person + verb"; Korean 안 = "peace" only inside 안전/평안 compounds, not bare 안), DO NOT add it as a separate meaning. Either:
...
- Concretely: imagine writing 2 simple examples using ONLY the bare headword in that sense, with the headword as a STANDALONE word (no compound, no fixed phrase wrap). If both feel forced or default to a compound/collocation, the sense is collocation-bound — EXCLUDE from meanings.
- COMPOSITIONAL TRAP (CRITICAL): a sense that only emerges when the headword combines with another morpheme to form a compound word is NOT a standalone sense.
  • Korean: 안 has a "peace/safety" sense ONLY inside compounds (안전 = safety, 평안 = peace). Bare 안 alone never carries that meaning in a natural sentence — exclude this sense. Same pattern for many Sino-derived monosyllables.
  • Generally: if the candidate sense always requires a partner morpheme to surface (e.g. always shows up in compound words and never as a bare-word predicate or object), it's compositional — exclude.
- This standalone gate runs BEFORE the inclusion-favored rule. Inclusion-favored governs WHICH attested senses to include; standalone determines whether a candidate sense is even attested as standalone. A sense that fails standalone is dropped regardless of the inclusion-favored bias.
- Net effect: meanings count should equal the number of standalone-usable senses. Words like "account" usually end up with 2 meanings (계좌 / 설명), not 3 (the rare "consider" sense is collocation-bound). Korean 안 ends up with 2 meanings (inside / proposal), not 3 (the "peace" sense is compound-bound).

ANTI-FABRICATION SECONDARY MEANING (CRITICAL — targets DUBIOUS senses only, not all secondary):
...
INCLUSION-FAVORED FOR ATTESTED POLYSEMY (NON-NEGOTIABLE COUNTER-BALANCE — anti-fabrication MUST NOT be used to drop genuinely common secondary senses):
- Default direction is INCLUSION, not exclusion. Drop a sense ONLY when you can articulate a SPECIFIC dubious-pattern reason (literary metaphor not lexicalized in this language, English-homograph drift into the WORD_LANG, constituent-character compositional sense). "I'm not sure" / "less common" / "secondary" alone is NEVER a reason to drop.
- Practical bar: if you can construct ONE natural learner-friendly example demonstrating a candidate secondary sense within ~2 attempts, that sense is attested enough — INCLUDE it. The fact that you can produce the example demonstrates the sense is real and usable.
- 5-second native-speaker test: read the bare headword in isolation. Within 5 seconds, would a native speaker think of ≥2 distinct everyday senses? If yes, INCLUDE those senses up to the 3-meaning cap. Bias toward yes — under-coverage of polysemy fails learners more than over-coverage.
- HIGH-PRIORITY INCLUSION PATTERNS (these MUST be included when applicable — the anti-fabrication rule does NOT override these):
  • A concrete TIME-OF-DAY noun in WORD_LANG that also lexically denotes the conventional meal eaten at that time (the "time-period word ALSO means the meal" pattern that exists across many languages — Korean, Japanese, Chinese, sometimes English). When BOTH senses are dictionary-attested for the exact input, INCLUDE both.
  • A noun headword that is ALSO an attested digit numeral in the WORD_LANG (Sino-numeral homographs in Korean: 일/이/삼/사/오/육/칠/팔/구/십 each have numeral senses regardless of which other nominal sense they primarily denote). When the input is one of these, the numeral sense is by definition attested — INCLUDE it as one of the meanings alongside any other attested senses.
  • Body-part / object nouns that double as units of measure (when both attested).
  • A noun headword that is ALSO a common given name / family name in the WORD_LANG culture, IF both senses are dictionary-attested (rare — only when both are common enough to appear in standard dictionaries).
- Decision is BINARY per sense: include ONLY IF dictionary-attested AND commonly used; drop ONLY IF you can name a specific dubious pattern. "When in doubt, return ONE meaning" applies ONLY when no other senses pass both tests — it does NOT apply to clear-polysemy attested senses where you simply hesitated.

MEANING-EXAMPLE PARITY (FINAL INVARIANT — overrides inclusion-favored when there's a conflict):
- The final meanings array count MUST equal the final examples array count (1:1, capped at 3). Both arrays are derived from the SAME set of "validated senses".
- VALIDATION PIPELINE (apply per candidate sense in this order):
  1. STANDALONE-EXAMPLE TEST: can you construct a natural learner-grade example demonstrating THIS sense with the BARE HEADWORD as a standalone word (not inside a compound)? Compound-only senses (Korean monosyllabic Sino-morphemes that only surface inside compounds — 안=peace via 안전/평안, 역=disease via 전염병, 도=way via 도덕, 김=Kim only in surname use, etc.) FAIL this test. Drop.
  2. ANTI-FABRICATION-SECONDARY: is this sense dictionary-attested AND commonly encountered? If dubious (literary metaphor not lexicalized, English-homograph drift, character-level decomposition), drop.
  3. HEADWORD-FORM SACRED CONSTRUCTION: try to construct the actual example, keeping the headword VERBATIM (no swap to native↔sino numeral, no swap to compound form). If you cannot produce a natural sentence under length / vocab-tier / counter-pairing constraints, drop.
  4. SURVIVE: sense is added to the final meanings array, and its constructed example becomes the corresponding examples array entry.
- If a sense passes steps 1-2 but fails step 3 (example cannot be constructed naturally with the sacred headword form), DROP THE MEANING from the canonical array. Do not list a meaning you cannot exemplify. Better to return fewer well-supported senses than to publish a meaning with no example, an awkward example, or an example that swaps the headword form.
- 컴포지셔널 트랩 SUMMARY (apply at step 1): a sense that only emerges when the headword combines with another morpheme into a compound word, and never appears as a bare-word predicate or object in standard sentences, is COMPOUND-BOUND. Examples of this trap (mention only structurally, not by specific word):
  • Sino-derived monosyllables in Korean whose abstract sense (peace/way/disease/morality/principle/period/region) only appears via compounds.
  • Latin / Greek roots in English (-graph, -logy, mono-, bi-) that have semantic content only via combination.
  • Constituent characters in CJK compounds (already covered by compositional unit rule).
  All COMPOUND-BOUND candidates fail the standalone gate. Drop without mercy — these are the most common over-inclusion error.

CHARACTER-DICTIONARY vs WORD-DICTIONARY (CRITICAL — applies to Sino-derived monosyllabic input in Korean / Japanese / Chinese):
- Sino-Han characters have rich semantic histories. CHARACTER DICTIONARIES (한자 사전 / 漢和辞典 / 漢字字典) list 5-15 glosses per character covering historical / abstract / compound-only senses. WORD DICTIONARIES (어휘 사전 / 国語辞典 / 词典) list the senses that the EXACT input string actually carries as a standalone word in modern usage.
- This product is a WORD-level vocabulary tool. Use WORD-DICTIONARY senses only. REJECT character-dictionary senses that only surface inside compounds.
- Two distinct rejection categories (BOTH must be applied):
  (a) PURELY ACADEMIC / LITERARY CHARACTER GLOSSES — senses that exist ONLY as character-dictionary entries and never carry independent semantic weight in modern sentences (often labeled with archaic / literary / classical / 古 / 漢 in dictionaries). These contribute NO real-world meaning to a bare monosyllabic headword. REJECT entirely (no meta-frame either).
  (b) COMPOUND-CONSTITUENT SENSES — the character's sense is widely known but exists exclusively via compound words (e.g. a character that contributes "history" only via the compound for "history", "disease" only via the compound for "epidemic", "peace" only via the compound for "safety"). The bare monosyllable cannot carry the sense in a sentence — the compound IS the word for that sense. REJECT from canonical meanings; if the user wants that sense, they should look up the COMPOUND, not the bare character.
- A few legitimate exceptions where bare monosyllables ARE the standalone word for the sense in modern usage: counter / unit senses (degree, hour, page), proper-noun-suffix senses where the character commonly suffixes onto place / person names (province / island suffix, surname). These CAN appear as canonical meanings BUT see the example-frame rule below.

EXAMPLE FRAME OPTIONS (apply at step 3 of validation pipeline — choose ONE frame per example based on the sense's nature):
- (i) NATURAL-USAGE FRAME (default for most senses): the headword appears as a regular word in a natural sentence demonstrating its meaning ("나는 **사과**를 먹어요" / "**책**이 무겁다"). Use whenever the sense is a normally-used standalone word.
- (ii) COUNTER-IN-CONTEXT FRAME (for unit / counter senses): the headword appears as a counter following a natural numeral, embedded in a natural context. Pattern: <subject> <natural-numeral> <headword>(-particle) <predicate>. Use for senses like "degree as measure unit" / "hour as time-of-day" / "serving as quantity" — show the unit in use, not in meta-talk.
- (iii) META-EXPLANATORY FRAME (for suffix-only senses where standalone usage is unnatural but the sense is real and commonly recognized): the example uses the meta pattern "<compound containing the headword>의 **<headword>**은/는 <sense-equivalent>을/를 뜻한다 / 의미한다" — explaining what the headword denotes within compounds. Use ONLY when the headword genuinely appears in modern compounds carrying this sense (e.g. place-name suffixes) AND standalone-word usage would be unnatural. The headword is marked as the headword normally. Do NOT abuse this frame for compound-constituent senses that should be rejected (those go to step 1 reject).

KOREAN STATE-ADJECTIVE SUBJECT (CRITICAL when WORD_LANG is Korean and a meaning is a state / sensory adjective describing a bodily or psychological state):
- Korean state-adjectives describing physical / sensory / emotional states (full / hungry / hurt / comfortable / uncomfortable / nervous / calm) take the AFFECTED BODY PART or PSYCHOLOGICAL STATE as their grammatical SUBJECT (with 이/가). The person experiencing the state is the TOPIC (with 은/는), not the subject of the adjective.
- Pattern: "<topic-person>은/는 <state-bearer>이/가 <state-adjective>" or just "<state-bearer>이/가 <state-adjective>".
- WRONG: "나는 **부르다**" (using 나 as direct subject of state-adj 부르다 with no 배). The structure with no state-bearer noun is ungrammatical for these adjectives.
- RIGHT: "**배**가 **부르다**" (the stomach is full) or "나는 **배**가 **부르다**" (as for me, my stomach is full).
- Same pattern for: 고프다 (배가 고프다), 아프다 (머리/배/X가 아프다), 시원하다 / 답답하다 (가슴이 / 마음이), 떨리다 (손이 / 몸이).
- When generating an example for these state-adjectives, the state-bearer noun MUST appear in the sentence; the marker is still on the headword adjective.

KOREAN BARE STEM SCOPE (NON-NEGOTIABLE when WORD_LANG is Korean and the input is a bare stem without the -다 ending):
- Korean verbs and adjectives are dictionary-listed in their -다 form (먹다 / 가다 / 팔다 / 따뜻하다). The bare stem (먹 / 가 / 팔 / 따뜻) WITHOUT -다 is a bound morpheme, NOT a standalone lexeme. It cannot carry the verb/adjective sense by itself.
- When the input is a bare stem, return ONLY senses that exist as standalone nouns / particles / fixed expressions / numerals for that exact bare-stem string. Do NOT include the verb / adjective sense that would belong to the corresponding -다 form.
  • Input "팔" → return noun senses only: arm, Sino-numeral 8. Do NOT include "to sell" (that sense belongs to 팔다, not 팔).
  • Input "팔다" → return verb sense: to sell. Do NOT include the unrelated noun senses of bare 팔 (different headword).
  • Input "먹" → return non-verb senses if any (rare); usually empty meanings + note="non_word". The verb "to eat" belongs to 먹다.
  • Input "따뜻" → adjective stem; not a standalone lexeme. Empty meanings + note="non_word" unless a noun sense exists.
- Self-check: if input has no internal "-다" suffix AND no internal Hangul space, ask "would a Korean dictionary list this exact bare string as a headword?". If only the -다 form appears in the dictionary, the bare stem is OUT of scope as a verb/adjective. Strip verb/adjective senses; keep noun/expression senses if attested.
...
- SLANG / VULGAR / DEROGATORY SECONDARY EXCLUSION (NON-NEGOTIABLE — this product is a LANGUAGE LEARNING TOOL, not a reference dictionary): when a word has a primary clean sense plus a secondary slang / vulgar / derogatory sense (year word that doubles as a female slur, dog word that doubles as a derogatory intensifier, pepper word that doubles as a sexual euphemism, mop/rag word that doubles as a female slur, etc.), INCLUDE ONLY THE PRIMARY CLEAN SENSE in the meanings array. Drop the secondary slang/vulgar sense entirely — do NOT include it with a register tag, do NOT include it at all. Normal emotional / negative vocabulary (anger, sadness, frustration, dislike) is NOT slang and is INCLUDED normally. Informal but non-vulgar colloquialisms (everyday neutral speech, common youth speech that isn't crude) are NOT slang and are INCLUDED normally.
- PRIMARY-SLANG OUT-OF-SCOPE (NON-NEGOTIABLE): when a word's primary and dominant sense is itself slang/vulgar/profane/derogatory (curse words used as interjections, scatological terms, sexual vulgarity, racial/ethnic/gender slurs, period-correct hate slogans), return empty meanings with note="non_word" — these are outside the educational learning vocabulary scope. NEVER produce a register-tagged "vulgar interjection / strong profanity" definition; that normalizes the term. The empty-meanings + note shape signals "not in our learning scope" cleanly.
...
- Profanity / slurs / hate slogans: out-of-scope per the SLANG / VULGAR exclusion rule above. Return empty meanings + note="non_word". The learning-tool positioning takes precedence over dictionary-faithful coverage.
...
FINAL NATIVE-SPEAKER NATURALNESS CHECK (CRITICAL — silently simulate a native speaker reading your output before emitting):
- For EACH meaning in "meanings": simulate a native speaker of WORD_LANG hearing the bare headword in isolation. Would they list this sense among the commonly-recognized senses of THIS EXACT bare string? Or would they say "that sense exists only inside compound word X / only as a character-dictionary gloss / only in classical literature / only as an archaic specialized term"? If the latter, the sense fails the native-speaker test — REMOVE IT from the array. This check is the FINAL filter — it overrides inclusion-favored bias when a sense fails.
- For EACH meaning_translated in "meanings_translated": simulate a native speaker of TARGET_LANG reading the translated definition. Is it natural concise dictionary-style phrasing, or stiff / overly-literal / paraphrastic? If unnatural, rewrite to the natural equivalent. Specifically check for translationese (literal word-by-word translation that no native speaker would write).
- The naturalness check is binary: include if a native speaker would readily accept; remove/rewrite if they would flag it as "not natural", "not how we say it", "academic only", or "compound-only".`;
...
// ============================================================
// PROMPT 2a': ALL_EXAMPLES — all examples in a single call (v3-2026-05-17)
// ============================================================
// Replaces PER_MEANING_EXAMPLE for v3 quality. v2's per-meaning split was
// designed to make cross-tagging impossible by giving the model only one
// meaning in context, but in practice the model still defaulted to the
// more familiar sense (아침=morning slot got the breakfast example), so
// canonical consistency broke anyway. v3 reverts to v1's "all meanings in
// one call" design with explicit meaning_index per example AND an explicit
// pre-emit coherence check.
//
// Architectural: canonical examples are SOURCE_LANG only. Translations
// (target_lang) are handled by buildTranslateSentenceSystemPrompt() in a
// separate downstream call. This preserves v2's canonical-consistency
// invariant (same source sentence appears across all target_langs).

const ALL_EXAMPLES_STATIC = `You produce a SET of example sentences demonstrating SPECIFIC senses of a vocabulary headword, all in WORD_LANG only.

Input (user message): HEADWORD and a list of MEANINGS with indices.

Output JSON: { "examples": [ { "sentence": string, "meaning_index": number }, ... ] }
- WORD_LANG only. NO translation field — translations are generated by a separate downstream call.
- "meaning_index": 0-based index of the meaning each sentence demonstrates. EVERY example MUST have a valid meaning_index pointing at the input meanings.

QUANTITY PER MEANING (NON-NEGOTIABLE — strict 1:1 schedule):
- 1 meaning: exactly 1 example with meaning_index 0.
- 2 meanings: exactly 2 examples — 1 each for meaning_index 0 and 1.
- 3 meanings: exactly 3 examples — 1 each for meaning_index 0, 1, 2.
- 4+ meanings: exactly 3 examples — 1 each for 0, 1, 2 (later meanings unsupported).
- Self-check before emitting: tally per meaning_index, confirm exactly one example per meaning.
- If a meaning genuinely cannot support a useful example (collocation-only sense, sensitive content with no metalinguistic template), drop that slot rather than padding with forced / unrelated content. Fewer correct beats more noisy.

MEANING-EXAMPLE COHERENCE (CRITICAL — the model's most frequent failure mode):
- For each emitted sentence, the SENTENCE'S DEMONSTRATED SENSE must match the listed definition for that meaning_index.
- Pre-emit check per sentence (mandatory): "If a learner reads ONLY this sentence with no other context, which of the input meanings would they naturally infer the headword carries here?" The answer MUST equal the meaning_index you are about to assign.
- DO NOT default to the headword's most familiar sense regardless of which meaning_index slot you are filling. Each slot demands a sentence that distinctively shows THAT slot's sense.
- The model's known failure: for a word with senses [morning, breakfast], producing a "I eat breakfast" sentence and assigning it meaning_index=0 (morning). That is wrong — the sentence demonstrates BREAKFAST, so it belongs to whichever index lists "breakfast" as the definition, NOT to morning.

EXAMPLE SHAPE (always simple, applies to every sentence):
- Length: 4–7 words (Latin) / 5–10 chars (CJK). Hard ceiling: 10 words / 15 chars, reserved for fixed multi-word expressions only.
- Shape: Subject + Verb + (Object). No subordinate clauses, no scene-setting, no adjective stacking.
- Supporting vocab MUST come from the most common ~1,000 words of WORD_LANG (children's-book register: pronouns, "is/has/eats/sees", "person/book/house/day", "big/small/good"). Never use intermediate / advanced supporting vocab.
- Tone: warm, neutral, daily-life scenes. For inherently negative / clinical senses (die, war, tax), use a dignified neutral scene, not a saccharine one.
- Polarity: AFFIRMATIVE usage. Do not use negation that flips the meaning (不 / ne...pas / 안 / ない / not) unless the headword INHERENTLY expresses negation.
- TERMINAL PUNCTUATION (CRITICAL): every source sentence MUST end with appropriate sentence-ending punctuation. Korean and Japanese use . / 。/ ! / ? as written. Latin scripts use . / ! / ?. No trailing whitespace, no missing terminator. The translation field also ends with punctuation. A sentence without terminal punctuation is incomplete output — append it.

VERB-SENSE NATURAL ARGUMENT (CRITICAL — applies to every example whose corresponding meaning is a verb / verb phrase):
- A verb sense in the example MUST be demonstrated with its TYPICAL ARGUMENT (object / complement). A bare predicate without the verb's typical argument is grammatically valid but doesn't teach the sense.
  • If the verb meaning is "to sing", the example needs an object: "노래를 **부른다**" — NOT bare "**부른다**". The learner must see that "to sing" takes "song" as its object.
  • If the verb meaning is "to eat", the example needs an object: "**먹었다** 사과를" wrong (verb-final), "사과를 **먹었다**" right.
  • If the verb meaning is "to play (instrument)", the example needs an instrument object.
  • If the verb is intransitive (to walk, to run, to sleep), no object needed — natural subject suffices.
- Self-check: read the example, then ask "without seeing the meaning definition, would the learner know WHICH sense of this verb is being demonstrated?" If multiple verb senses could fit, you haven't disambiguated — add the typical argument.

PROPER NOUN / FAMILY-NAME / SURNAME SENSE FRAME (CRITICAL):
- When a meaning is a family name / surname / given-name sense (e.g. Korean 김 / 박 / 이; English Smith / Jones), the example MUST use a NEUTRAL DESCRIPTIVE FRAME explaining the name as an attribute. DO NOT use the address frame "Mr./Mrs. <name>".
  • CORRECT: "그 사람의 성은 **김**이다 (their surname is Kim)" / "이 분의 성은 **박**이에요" — the headword is presented as a piece of information about the person's name.
  • WRONG: "**김** 씨가 학교에 간다 (Mr. Kim is going to school)" — using the surname directly as an address form. In Korean, "X 씨" is borderline derogatory in many contexts (especially when X is a bare surname without given name); avoid it.
  • Same principle in other languages: present the name AS data ("her last name is Smith"), not AS address ("Mr. Smith said..."). The teaching goal is "this word is a surname", not "you should call someone by this surname".

HEADWORD-FORM SACRED — DROP RATHER THAN MIS-PAIR (overrides coverage guarantee):
- When the headword form constrains the example AND no natural learner-grade example can be constructed while keeping the headword verbatim:
  • For Sino-numeral headwords (일/이/.../십) whose dictionary sense is the numeral: the example must use a sino-compatible counter (분/원/페이지/회/호/인분/학년/도/등 etc.). 시 / 살 / 명 / 개 / 마리 / 채 / 대 etc. take NATIVE numerals and are INCOMPATIBLE with sino headwords — never pair them.
  • If you cannot find a natural sino-counter context that fits a 5-10-character learner sentence, DROP that meaning's example slot rather than producing a sentence with a mismatched counter or swapping the headword form.
- When the headword has multiple senses and an example for a specific sense would require swapping the headword to a related-but-different lexeme (e.g. headword 역 demonstrating "role" sense, but the natural collocation "역할" replaces 역): use the bare headword in a natural frame for that sense ("그는 중요한 **역**을 맡았어요" — the role sense in 역할 is conventionally expressible via bare 역 with 맡다), OR drop the slot if no such frame exists. Never substitute the related-but-different word inside the marker.
- Coverage guarantee says default-to-produce, but it yields to LEMMA IDENTITY when there's a conflict — accuracy beats coverage.

HEADWORD MARKER (NON-NEGOTIABLE — the SECOND most frequent failure mode):
- Wrap the headword (in its inflected/conjugated form for this sentence) in EXACTLY ONE pair of **...**.
- The marker MUST be on the HEADWORD or its valid inflection — NEVER on an adjacent verb, adjective, or particle.
  • WRONG: "나는 아침을 **먹는다**" (headword 아침, marker on 먹는다 — adjacent verb)
  • RIGHT: "나는 **아침**을 먹는다" (marker on the headword 아침; the particle 을 stays outside)
- Include the FULL inflected form inside the markers. NEVER let suffixes/endings leak outside.
  • en: -s/-es/-ed/-ing/-d/-ies/-ier — all inside. "She **promotes** him" not "She **promote**s him".
  • fr/es/it/pt: full conjugated form inside. "Elle **promeut**" not "Elle **promo**ut".
  • de: full conjugated form (-e/-st/-t/-en/-te) inside.
  • ko: stem + -다 inside (when dictionary-form). "**승진하다**" not "**승진**하다". Particles (을/를/이/가/은/는/에/의/로/와/과) stay OUTSIDE.
  • ja: full kanji + okurigana inside. Particles (は/が/を/に/へ/で/と) OUTSIDE.
  • zh: all characters of the multi-char word together. Structural particles (的/了/过/着) OUTSIDE.
- Multi-word headwords ("feu rouge", "look up", proverbs): wrap the ENTIRE phrase as one unit.
- LEMMA IDENTITY: the bolded substring is the SAME lexeme as the headword. Never a same-spelled different word.

KOREAN VERB-FINAL (NON-NEGOTIABLE when WORD_LANG is Korean):
- Every clause's main verb (or verb+ending) MUST appear at the END of the clause, after the subject and any object / location / manner / time adjuncts. Producing English-style SVO Korean is grammatically wrong.
- Correct: Subject + Object/Adjuncts + Verb. "나는 사과를 먹는다" / "그는 학교에 가요" / "우리는 영화를 본다".
- Wrong: "나는 먹는다 사과를" / "그는 가요 학교에" — silently regenerate.

KOREAN NUMERAL-COUNTER COMPATIBILITY (NON-NEGOTIABLE when WORD_LANG is Korean and any numeral appears in the example):
- Korean has two parallel numeral systems with HARD pairing rules to specific counters. Mixing produces ungrammatical sentences.
- NATIVE numerals (하나/둘/셋/넷/다섯/여섯/일곱/여덟/아홉/열, with prenominal forms 한/두/세/네) pair ONLY with NATIVE counters:
  • 명 (people), 사람 (person), 마리 (animal), 개 (general object), 살 (age), 권 (book), 잔 (cup/glass), 병 (bottle), 채 (building), 대 (vehicle/device), 송이 (flower), 켤레 (pair of footwear), 그릇 (bowl), 자루 (stick/pencil), 그루 (tree), 가지 (kind), 번 (occurrence, with native), 시 (hour-of-day: 한 시 / 두 시 / 세 시), 달 (month-as-duration: 한 달 / 두 달).
- SINO-Korean numerals (일/이/삼/사/오/육/칠/팔/구/십/백/천/만) pair ONLY with SINO-borrowed / numeric counters:
  • 분 (minute), 초 (second), 원 (currency), 페이지/쪽 (page), 층 (floor), 호 (edition/issue/room number), 회 (occurrence, formal), 인분 (food serving), 학년 (school grade), 도 (degree), 월 (calendar month: 일월/이월), 년 (year as unit: 일 년/이 년), 미터/킬로미터/그램 (units).
- The pairing direction is HARD. Example sentences that demonstrate a Sino numeral as headword (일/이/삼/사/오/육/칠/팔/구/십) MUST use a Sino-compatible counter, not a native one.
  • WRONG: "**팔** 명이 왔다" — Sino 팔 + native counter 명. Use native 여덟 with 명, or pair 팔 with Sino counter.
  • RIGHT for Sino 팔 demonstrating "eight": "**팔** 시에 만나요" (8 o'clock), "**팔** 인분 주문했어요" (eight servings), "**팔** 페이지를 봐요" (page 8).
  • WRONG: "여덟 분 후에 와요" — Native 여덟 + Sino counter 분. Should be "팔 분 후에 와요" (eight minutes).
  • RIGHT for Native 여덟 demonstrating "eight": "**여덟** 명이 왔어요" (eight people), "**여덟** 살이에요" (eight years old), "강아지 **여덟** 마리가 있어요" (eight puppies).
- When the headword IS the numeral itself (Sino 일~십 or Native 하나~열), the example sentence MUST pair the numeral with a compatible counter from the lists above. NEVER use the wrong system's counter.
- HEADWORD-FORM SACRED (overrides everything): when the headword is a Sino numeral (e.g. 팔 = eight), the example MUST keep "팔" verbatim — NEVER swap to the native equivalent "여덟" to satisfy a counter pairing. Same in reverse: native headword 여덟 stays as 여덟, never swapped to Sino 팔. The two numeral systems are DIFFERENT LEXEMES; replacing one with the other violates LEMMA IDENTITY.
  • CORRECT for Sino-headword 팔 demonstrating "eight": "**팔** 시에 만나요" / "**팔** 인분 주문했어요" / "**팔** 페이지를 봐요" / "지금 **팔** 시예요".
  • CORRECT for Native-headword 여덟 demonstrating "eight": "**여덟** 명이 왔어요" / "**여덟** 살이에요" / "사과 **여덟** 개 사세요".
  • WRONG: headword 팔, example "**여덟** 시에 일어났어요" (sacred headword form was swapped — fix by using "**팔** 시에 일어났어요").
- If no natural example fits the headword's numeral system, DROP that slot rather than swapping the headword form. Coverage loss is far less harmful than producing a misleading sentence that doesn't actually contain the headword.
- Self-check before emitting any Korean example with a numeral: (a) does the SENTENCE contain the EXACT headword token (no system swap)? (b) is the counter on the matching list for that numeral system? If either fails, rewrite or drop.

FRENCH ELISION (NON-NEGOTIABLE when WORD_LANG is French):
- le/la/de/je/ne/que/ce/se/me/te (and si before il/ils) contract to l'/d'/j'/n'/qu'/c'/s'/m'/t' before vowel-initial and h-muet words. "Je écris" / "Le étoile" / "Que il" are hard errors — always "J'écris" / "L'étoile" / "Qu'il".

SLANG / VULGAR EXCLUSION (per learning-tool positioning):
- If any meaning slipped through with a slang / vulgar / derogatory sense, drop the example slot for that meaning rather than producing the example. The slang exclusion rule on the canonical side should have prevented this, but if it didn't, fail safe here.
- Slurs / strongest profanity headwords: return examples=[] (the canonical analysis should already have returned empty meanings + note="non_word", so this path should not fire).

SENSITIVE CONTENT (apply when the user prompt contains "SENSITIVE LOOKUP" or when the headword matches a known sensitive entity):
- Use ONLY metalinguistic templates that demonstrate USAGE / GRAMMAR ONLY, communicating as little semantic content about the entity as possible.
  • "I read the word X in a book" / "We learned about X in geography class" / "The textbook mentions X" / "I wrote X in my notebook" / "X appears on this map" / "I looked up X in the dictionary"
- NEVER write "X is famous for Y" / "X is known for Y" / "I want to visit X" / any predicate that describes properties (history/beauty/size/importance/sovereignty/political status) of the entity.
- If no template fits the sensitive lookup: drop the slot (fewer examples beat risky ones).

CONTENT NEUTRALITY (every example, regardless of sensitivity flag):
- Stay in the realm of generic mundane daily-life scenes. NEVER reference specific real-world sensitive topics: territorial / naming disputes (East Sea / Dokdo / Senkaku / Crimea / Kashmir / Jerusalem etc.), identifiable real political figures (current or historical), specific wars / atrocities / contested events (WWII specifics / Holocaust / comfort women / Nanjing / 9/11 / Cultural Revolution / Tiananmen), religious doctrine / comparison / judgment, ethnic / national / racial stereotypes (even positive-sounding), real political parties / movements / slogans, real-name brands / celebrities / athletes (use fictional substitutes unless the headword IS the brand/person), recent disasters / crimes / tragedies.
- If a sensitive headword naturally invites such a scene (war, president, religion, border, refugee, massacre), construct a generic / abstract / historically-distant scene instead.

COVERAGE GUARANTEE:
- Default is ALWAYS to produce the scheduled number of examples even when the simplest 4-7-word structure is hard. A meaning that survives in the canonical meanings array deserves an example.
- Empty slots reserved for (a) sensitive content with no metalinguistic template fit, (b) slurs / strongest profanity, (c) slang/vulgar sense that should have been excluded canonically.
- For idioms / phrasal verbs / fixed expressions / multi-word lemmas: use the higher 10-words / 15-chars ceiling freely.
- When in doubt: produce the most ordinary natural sentence the lemma can carry. A slightly less simple example is FAR more useful than no example.

FINAL VERIFICATION (silent before emitting JSON):
1. Tally examples per meaning_index — matches the schedule exactly?
2. For each example, run the coherence check: does the SENTENCE demonstrate the SENSE listed at that meaning_index?
3. For each example, the marker is on the HEADWORD (or its valid inflection) — NOT on an adjacent verb / adjective / particle?
4. Length within limits, supporting vocab basic, polarity affirmative, korean verb-final / french elision satisfied if applicable?
5. Source sentence ends with appropriate terminal punctuation (. / ! / ? / 。 / ! / ?)?
6. No translation field present in any example?

FINAL NATIVE-SPEAKER NATURALNESS CHECK (CRITICAL — silently simulate a native speaker reading each sentence before emitting):
- For EACH example sentence: imagine a native speaker of WORD_LANG reading the sentence aloud. Would they say "this is a natural sentence a Korean/Japanese/etc. would actually produce", or would they say "this sounds forced / grammatically odd / lexically wrong / uses the word in an unnatural way"? If the latter, REWRITE the sentence or DROP the slot (better fewer correct examples than awkward ones).
- Specifically catch these failure modes:
  • State-adjective without state-bearer subject (e.g. "나는 부르다" lacking 배가; "그는 아프다" lacking 머리가 / 몸이 etc.) — REWRITE with the state-bearer noun.
  • Character-dictionary sense forced into a sentence where the compound word would be used instead ("감기에 역이 생겼어요" — natural speaker would say "감기에 걸렸어요" using 감기, not 역). REWRITE or DROP.
  • Numeral-counter mismatch (sino numeral with native counter or vice versa) — REWRITE with matching counter or DROP.
  • Lexically wrong word used in place of the headword (substituting "역할" inside the marker when headword is "역") — REWRITE with bare headword.
  • Awkward stand-in usage of a compound-suffix-only sense as a bare noun ("성공하는 도를 배웠어요" — native speaker would say "성공하는 길/방법을 배웠어요"). REWRITE with meta-frame or DROP.
- The naturalness check is the FINAL gate. If you cannot rewrite to natural usage while keeping the headword form sacred, DROP the example (and drop the meaning per the meaning-example parity invariant).`;
...
export function buildAllExamplesUserPrompt(
  ...
  lines.push(
    "",
    "Produce the examples per the quantity schedule. Each example MUST satisfy the coherence + marker checks before emission.",
  );
  return lines.join("\n");
}
