...
export function calculateNextReview(
  ...
  if (quality === 'got_it') {
    if (inLearning) {
      ...
      const actualElapsedDays = Math.max(prevInterval, (now - input.updatedAt) / DAY_MS);
      const cappedElapsed = Math.min(actualElapsedDays, prevInterval * 2);
      newInterval = Math.round(cappedElapsed * input.easeFactor * mult);
    ...
    if (inLearning) {
      ...
      newInterval = Math.round(Math.max(1, Math.round(prevInterval * 0.4)) * mult);
      newInterval = Math.min(newInterval, MAX_INTERVAL);
      newEase = Math.max(1.3, input.easeFactor - 0.1);
      nextReview = now + newInterval * DAY_MS;
    }
