...
// Schedule: exactly 1 example per meaning. Example count == meaning count
// (1:1). No separate cap — the meaning count is the natural ceiling.
// The AI may under-deliver for hard senses; that's acceptable, but we
// never accept over-claimed examples for non-existent meanings.
...
export function balanceExamples(
  ...
  // 1 example per meaning. The idx < meaningCount filter + seen-set dedup
  // naturally bound output to meaningCount. No additional cap needed.
  ...
  // Sort by meaningIndex so the UI renders examples in the same order as
  // the meanings they belong to. The LLM occasionally emits per-meaning
  // examples out of order (e.g. [idx=1, idx=0]); without sorting, the
  // wordlist card shows meaning1, meaning2, example2, example1.
...
export function realignExamplesByPos(
  ...
  // Bounds-guard: an example may have meaningIndex >= meanings.length if
  // normalizeResult deduped/dropped a meaning upstream. Skip those — they
  // are handled by the orphan-drop pass downstream.
  const inBounds = (idx: number): boolean => idx >= 0 && idx < meanings.length;
  for (let i = 0; i < work.length; i++) {
    const ci = work[i].meaningIndex ?? 0;
    if (!inBounds(ci)) continue;
    if (matchesSlot(i, ci)) continue;
    for (let k = i + 1; k < work.length; k++) {
      const ck = work[k].meaningIndex ?? 0;
      if (!inBounds(ck)) continue;
