// Invalidate word_entries + word_translations cache for ko/ja/zh-CN/en
// (and existing es/fr/de/it from prior session if still stale) so the
// deployed v2 function regenerates with the consolidated case-routed
// prompts (EN regressions fixed, JA Phase 1 + retunes, ZH Phase 1, KO
// SynAnt case routing).
//
// Strategy: set prompt_version='outdated-2026-05-19-evening'. Lookup
// filters by current PROMPT_VERSION_V2 — outdated rows invisible →
// regenerated lazily on next user lookup.
...
const LANGS = ['ko', 'ja', 'zh-CN', 'en'];
const NEW_TAG = 'outdated-2026-05-19-evening';
const PAGE = 1000;
...
console.log(`\nDone. Next lookups for ${LANGS.join('/')} regenerate with the consolidated case-routed prompts.`);
