...
import { buildUserPrompt, buildEnrichUserPrompt, buildMarkerFixPrompt, buildMarkerBackfillPrompt, buildSentenceBackfillPrompt, buildIpaOnlyPrompt, getSystemPrompt, LANG_NAMES } from "../_shared/prompts.ts";
...
function validateInput(body: unknown): WordLookupRequest {
  ...
  // Optional proficiency hint (curation use): tells enrich mode the headword's
  // proficiency tier so example sentences stay at the right level.
    ...
    async start(controller) {
      ...
      try {
        ...
        try {
          ...
          // Korean cardinal-numeral dual-form override (1–10): the model
          // is non-deterministic at producing both native and Sino forms
          // and sometimes mixes systems across consecutive lookups. Inject
          // the canonical pair so consecutive number searches stay
          // consistent for learners.
          const dualNumStream = getDualNumeralOverride(request.sourceLang, request.targetLang, request.word);
          if (dualNumStream) {
            result.meanings = dualNumStream;
...
Deno.serve(async (req: Request) => {
  ...
  if (isTranslate) {
    ...
    const tCacheKey = `${request.word.trim().toLowerCase()}|${request.sourceLang}-${request.targetLang}|translate|v3|${DEFAULT_MODEL}`;
...
5. CASE-INDIFFERENT INPUT (when ${fromName} uses capitalization to mark proper nouns): treat the user's input as case-insensitive — uppercase, lowercase, and titlecase variants of the same letter sequence are the SAME query. Before deciding the candidate set, mentally consider every casing of the input. If the letter sequence is also a standard proper noun (calendar period, weekday, given/family name, place, brand) in addition to a common-word sense, BOTH senses must appear as candidates regardless of how the user typed the input. The user's casing never narrows the eligible senses.
6. CANONICAL CASING ON OUTPUT (when ${toName} uses capitalization to mark proper nouns): each "headword" must use the spelling a native writer would use in print — proper-noun candidates (calendar period, weekday name, given/family name, place, brand) are capitalized; common-word candidates are lowercase. Never lowercase a proper noun and never uppercase a common word, irrespective of input casing.
7. NO REGISTER/SYNONYM PADDING — if two near-synonymous ${toName} words convey the same sense at the same register, return only the most everyday one as a single candidate. Filling multiple candidate slots with synonyms of the same sense is forbidden — those slots must instead go to genuinely DISTINCT senses (different homonyms, different polysemic meanings) if any exist. A second candidate is justified only when its sense is meaningfully different from the first.
...
- WHEN candidates.length > 1: EVERY candidate (including the most common/"default" one) MUST carry a non-empty disambiguating hint. Picking just the alternates and leaving the primary blank is invalid — the learner cannot tell which sense they're choosing if only some candidates are labeled. Self-check: if candidates.length > 1, count how many hints are non-empty; if any are empty, fill them with a short ${fromName} clarifier before emitting.
...
5. If candidates.length > 1, does EVERY candidate have a non-empty hint? If any hint is empty/missing, fill it.`,
  ...
  try {
    ...
    // Korean cardinal-numeral dual-form override — see streaming path.
    const dualNumNonStream = getDualNumeralOverride(request.sourceLang, request.targetLang, request.word);
    if (dualNumNonStream) {
      result.meanings = dualNumNonStream;
      result.confidence = Math.max(result.confidence ?? 0, 95);
      result.note = undefined;
    }
    ...
    if (isEnrich && result.examples?.length) {
      ...
      // Strip orphan ** markers — the model occasionally produces "headword**"
      // (closing only, missing opening) which currently evades backfill because
      // .includes("**") is satisfied. Treat any string lacking a proper
      // **X** pair as marker-less so backfill can rebuild it cleanly.
      const stripOrphanMarkers = (s: string): string =>
        /\*\*[^*]+\*\*/.test(s) ? s : s.replace(/\*\*/g, "");
      result.examples = result.examples.map((ex) => ({
        ...ex,
        sentence: ex.sentence ? stripOrphanMarkers(ex.sentence) : ex.sentence,
        translation: ex.translation ? stripOrphanMarkers(ex.translation) : ex.translation,
      }));

      // Sentence-marker backfill — same idea but for source sentences.
      // The main fix pass relies on the AI to mark conjugated/inflected
      // forms, but in inflected source languages (fr/de/es/it/pt/ru) the
      // AI sometimes returns sentences without **. This narrow pass forces
      // markers onto those source sentences using the headword + meanings.
      const needSentBackfill = result.examples.filter((ex) => ex.sentence && !ex.sentence.includes("**"));
      if (needSentBackfill.length > 0 && meanings && meanings.length > 0) {
        try {
          const sentPrompts = buildSentenceBackfillPrompt(
            request.word,
            request.sourceLang,
            request.targetLang,
            meanings,
            needSentBackfill.map((ex) => ({
              sentence: ex.sentence,
              translation: ex.translation,
              meaning_index: ex.meaningIndex,
            })),
          );
          const sentResp = await callOpenAiForWordLookup({
            systemPrompt: sentPrompts.system,
            userPrompt: sentPrompts.user,
            apiKey: openaiKey,
          });
          const sentData = sentResp.result as unknown as {
            examples?: Array<{ sentence?: string }>;
          };
          const fixed = sentData?.examples ?? [];
          if (fixed.length === needSentBackfill.length) {
            let bi = 0;
            result.examples = result.examples.map((ex) => {
              if (ex.sentence && !ex.sentence.includes("**")) {
                const s = fixed[bi++]?.sentence;
                if (typeof s === "string" && s.length > 0 && s.includes("**")) {
                  return { ...ex, sentence: s };
                }
              }
              return ex;
            });
          }
          totalTokensIn += sentResp.usage.prompt_tokens;
          totalTokensOut += sentResp.usage.completion_tokens;
          totalCost += sentResp.costUsd;
        } catch (err) {
          console.warn("sentence backfill failed:", err instanceof Error ? err.message : err);
        }
      }
    }
